...
%without the ground effect.

%INPUTS:
%Number of panels, Chord, Angle of attack (also possible to set a range of
%Aoa for Cl-alpha curve), U_infinity, Alpha, Distances between airfoils
...
title(strcat('Centroids and Airfoil',{' '},num2str(airfoil(1))))
...
title(strcat('Centroids and Airfoil 00',num2str(airfoil(2))))
legend('panels','control points')
hold off
% saveas(gcf, 'Tandem geometry singularly','png')


% --------- Show the aerofoil TOGHTTHER: ---------
figure()
hold on
plot(x(1,1:(n(1)+1)),z(1,1:(n(1)+1)),'k','Linewidth',1.2)
plot(x(2,1:(n(2)+1)),z(2,1:(n(2)+1)),'k','Linewidth',1.2)
title('Airfoils in tandem configuration')
axis equal
grid on
hold off
% saveas(gcf, 'Tandem geometry togheter','png')
...
[U_source1,U_vortex1] = source_vortex_velocity(x_c(1,:),z_c(1,:),x(1,:),z(1,:),beta(1,:),q(:,1),gamma(1),l_panel(1,:));
[U_source2,U_vortex2] = source_vortex_velocity(x_c(2,:),z_c(2,:),x(2,:),z(2,:),beta(2,:),q(:,2),gamma(2),l_panel(2,:));
...
%     [Cp(i,:),Cl(i),Cm_LE(i)] = flow_parameters(beta(i,:),n(i),l_panel(i,:),x_c(i,:),z_c(i,:),c(i),q(:,i),gamma(i),u_s,v_s, u_v,v_v,U_infinity,alpha);
...
title('C_p distribution of tandem configuration foils','FontSize', 15)
...
grid on
axis on
% saveas(gcf, 'Cp togheter tandem','png')
...
plot(x(1,1:(n(1)+1)),z(1,1:(n(1)+1)),'k','Linewidth',1.5)
plot(x(2,1:(n(2)+1)),z(2,1:(n(2)+1)),'k','Linewidth',1.5)
set (gca,'Ydir','reverse')
title('Pressure coefficient','FontSize', 15)
legend('Bottom wing','Top wing','Bottom tail','Top tail')
xlabel('chord','FontSize', 10,'fontweight','bold')
ylabel('C_p','FontSize', 10,'fontweight','bold')
grid on
axis on
% saveas(gcf, 'Cp singularly tandem','png')
...
zm = linspace(-0.8,0.4,d); %y-dmension
[X,Y] = meshgrid(xm,zm);
...
for i = 1:length(xm)
    for j = 1:length(zm)
        if airfoil_mask(i,j) == 1
            ...
            vel_U(i,j) = U_source1(1)+U_source2(1)+U_vortex1(1)+U_vortex2(1)+U_infinity*cos(alpha);
            vel_V(i,j) = U_source1(2)+U_source2(2)+U_vortex1(2)+U_vortex2(2)+U_infinity*sin(alpha);
...
axis equal
hold off
%saveas(gcf, strcat('Streamlines Airfoil tandem',num2str(airfoil(1)),num2str(airfoil(2))),'png')
...
grid off;
hold off;
%saveas(gcf, strcat('Color map Airfoil tandem',num2str(airfoil(1)),num2str(airfoil(2))),'png')
