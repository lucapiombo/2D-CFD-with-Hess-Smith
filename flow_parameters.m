function [Cp,Cl,Cm_LE] = flow_parameters(beta,n,l_panel,x_field,z_field,c,q,gamma,u_s,v_s, u_v,v_v,U_infinity,alpha)
%--------------------------------------------------------------------------
...
for i =1:n
    for j = 1:n
        ...
        sorg(i) = sorg(i)+ q(j) * sum([u_s(i,j),v_s(i,j)].*t_hat(i,:));
        vortex(i) = vortex(i) + sum([u_v(i,j),v_v(i,j)].*t_hat(i,:));
        

    end
    freestream(i) = sum(U_infinity*([cos(alpha), sin(alpha)]).*t_hat(i,:));
    U_t(i) = freestream(i)+sorg(i)+gamma*vortex(i); %velocità U effettiva su ogni pannello
end 
